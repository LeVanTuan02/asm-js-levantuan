    ...
    async render(keyword, pageNumber) {
        ...
        <main>
            <section class="container max-w-6xl mx-auto px-3 pt-8 mb-5 text-center">
                ...
                <h1 class="text-[#D9A953] font-semibold text-3xl mt-1">
                    ${productList.length ? `Kết quả tìm kiếm "${keyword}"` : `Không tìm thấy sản phẩm nào phù hợp`}
                </h1>
            </section>
            ...
            <section class="container max-w-6xl mx-auto px-3 grid grid-cols-12 gap-6 mb-8">
                ...
                ${productList.length ? ProductContent.render(productList, currentPage, total, totalPage, start, limit, `product/search/${keyword}`) : ``}
            </section>
        </main>
        <!-- end content -->
